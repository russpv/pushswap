 ...
 * Apparently rotating nothing or one element is valid
 */
...
bool	rotate_stack(t_stack_ptr stack)
{
	...
	if (true == DEBUGGING)
		mydebug("Log: Rotating %d on s:%d(i:%d) p:%d\n", get_stack_num(stack, \
				0), stack->id, top_num_idx, top_part_id);
...
bool	rev_rotate_stack(t_stack_ptr stack)
{
	...
	if (true == DEBUGGING)
		mydebug("Log: Reversing %d on s:%d(i:%d) p:%d\n", get_stack_num(stack, \
				stack->size - 1), stack->id, last_num_idx, last_part_id);
...
bool	swap_stack(t_stack_ptr stack)
{
	...
	if (true == DEBUGGING)
		mydebug("Log: Swapping %d & %d on s:%d(i:%d & i:%d) \n", \
			get_stack_num(stack, 0), get_stack_num(stack, 1), stack->id, \
			second_num_idx, stack->idx[0]);
